...
#from al_dataset import TrainDataset, TTADataset, N_CLASSES, DATA_ROOT
#from al_transforms import get_train_transform, get_test_transform, get_test_tta_transform
from al_dataset import TrainDataset, N_CLASSES, DATA_ROOT
from al_transforms import get_train_transform, get_test_transform, get_test_tta_transform
...
from test_dataset import TTADataset,fivecrop_transform
...
def main():
    ...
    if args.mode == 'train':
        ...
        scheduler = ReduceLROnPlateau(optimizer, patience=args.patience, factor=0.1, verbose=True, mode='max', min_lr=2e-6)
        ...
        if args.two_stage == 1: #switch loss
            ...
            train_kwargs['optimizer'] = Adam(filter(lambda p: p.requires_grad, model.parameters()), 2e-5)
            train(n_epochs=8, **train_kwargs)
        # args.lr/=10
        ...
        load_model(model, run_root / ('best-model-%d.pt' % args.fold), multi2single=True)
        model.cuda()
        #validation_TTA(model, criterion, valid_loader, args, save_result=True)
        validation_TTA(model, criterion, tqdm.tqdm(valid_loader, desc='Validation', ascii=True), args, save_result=True)
