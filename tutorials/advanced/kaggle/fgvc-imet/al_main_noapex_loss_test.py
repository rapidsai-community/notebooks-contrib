...
from losses import FocalLoss2d, JointLoss, bce_with_pos_weight, binary_focal_loss, FbetaLoss, LovaszLoss
from utils import (
...
def main():
    ...
    #criterion = JointLoss(FbetaLoss(beta=2), nn.BCEWithLogitsLoss(), second_weight=0.5)
    if args.mode == 'train':
        ...
        scheduler = ReduceLROnPlateau(optimizer, patience=args.patience, factor=0.1, verbose=True, mode='max')
        ...
        if args.two_stage == 1: #switch loss
            ...
            train_kwargs['criterion'] = FbetaLoss(beta=3)
            train_kwargs['optimizer'] = Adam(filter(lambda p: p.requires_grad, model.parameters()), 1e-5)
            train(n_epochs=2, **train_kwargs)
        # args.lr/=10
